---
name: generate-image
description: ComfyUI FLUX.1 Schnell 모델로 Notion 스타일 미니멀 일러스트 이미지를 빠르게 생성합니다. 명언 영상의 배경 이미지가 필요할 때, 철학적/예술적 스케치 이미지를 만들 때 사용하세요.
allowed-tools:
  - Bash
  - Read
  - Write
...
ComfyUI 서버에 연결하여 FLUX.1 Schnell 모델로 빠른 이미지를 생성하는 스킬입니다.

## Instructions

1. **프롬프트 준비**: 영어로 씬 설명 작성
2. **스타일 적용**: Notion 스타일 미니멀 일러스트 자동 적용
3. **이미지 생성**: ComfyUI API를 통해 FLUX.1 Schnell 모델 실행 (4 steps)
4. **결과 저장**: PNG 형식으로 저장
...
| Steps | 4 | FLUX.1 Schnell 권장값 (4-8) |
...
1. **빠른 생성**: FLUX.1 Schnell은 4 steps로 빠른 이미지 생성
...
8. **Flux2C API 지원**: Mac Metal 가속으로 더 빠른 생성 (선택사항)
...
### 모델 없음
- FLUX.1 모델 확인: `python find_flux.py`
